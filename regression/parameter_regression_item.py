            ...
            for user,userScore in rateFreq.items(): 
                ...
                for weight in np.arange(0, 1.1 ,0.1): # arange exclude endPoint while linspace includes defaultly  
                    ...
                    for score,freq in userScore.items():    
                        sumMse+=(( weight*stats.norm(miu1, theta1).cdf(score)+ (1-weight)*stats.norm(miu2,theta2).cdf(score)- freq ) )**2 # /freq
        ...
        for j in range(1):
            ...
            for user,userScore in rateFreq.items(): 
                ...
                for weight in np.arange(0, 1.1 ,0.1): # arange exclude endPoint while linspace includes defaultly  
                    ...
                    '''这里需要check下data'''
                    for score,freq in userScore.items():    
                        sumMse+=((weight*stats.norm(miu1, theta1).cdf(score)- freq ) )**2 # /freq
